...
class PRISMTransformer(nn.Module):
    """
    PRISMT (Pattern Reconstruction and Interpretation with a Structured Multimodal
    Transformer) model for widefield calcium imaging reconstruction.
    Each scalar (brain area x timepoint) becomes a token.
    For 10 brain areas and 20 timepoints, this creates 200 tokens.
    """
...
def create_prismt_model(
    ...
    """
    Create and initialize a PRISMTransformer model.
    ...
    logger.info(f"Created PRISMT model with {sum(p.numel() for p in model.parameters()):,} parameters")
